...
const DefendantFilterAdmin = ({
  ...
  const mapFilterDefendantAdminToRequest = (values) => {
    let findType = dataDefendant.find((item) => item.type);
    if (!!findType) {
  ...
  const submitRenderFilter = async (values) => {
    ...
    await getDefendant(mapFilterDefendantAdminToRequest(values));
  };
  return (
...
export default connect(
  ...
  reduxForm({
    form: "DefendantFilterAdmin",
    validate,
  })(DefendantFilterAdmin),
);
