...
const DefendantTableAdmin = ({
  ...
  const handleOnSwitchDefendant = (value, e) => {
  ...
  const handleOnEditDefendant = (value) => {
    showEditDefendant(value);
  };

  const handleOnDeleteDefendant = (defendant) => {
    vex.dialog.confirm({
      message: `ยืนยันการลบ ${defendant.header} ?`,
      callback: function (value) {
        if (value) {
          loading(LOADING.deleteDefendant);
          deleteDefendant(defendant._id);
        }
      }.bind(this),
    });
  };
  const handleonShowDetail = (value) => {
    showDefendantDetail(value);
  };
      ...
      <tr key={value._id}>
        ...
        <td style={{ textAlign: "center" }}>
          <Row style={{ paddingLeft: "55px" }}>
            ...
            <Col sm={2}>
              <FontAwesomeIcon
                onClick={() => handleOnEditDefendant(value)}
                icon={faEdit}
                style={{ fontSize: 26 }}
              />
            </Col>
            <Col sm={2}>
              <Form>
                <Form.Check
                  ...
                  onClick={(e) => handleOnSwitchDefendant(value, e)}
                />
              </Form>
            </Col>
            <Col sm={2}>
              <FontAwesomeIcon
                onClick={() => handleOnDeleteDefendant(value)}
                icon={faTrash}
                style={{ fontSize: 26 }}
              />
            </Col>
  ...
  const handleSelectPage = (pageKey) => {
    let findType = dataDefendant.find((item) => item.type); //typeที่มาจากการกด tab
    ...
    if (!!formStates) {
      ...
      window.scrollTo(0, 0);
      loading(LOADING.getDefendant);
      getDefendant({ pageKey, selectPageType: findType.type });
    }
  };
...
function mapStateToProps(state) {
  return {
    formStates: getFormValues("DefendantFilterAdmin")(state),
    pagecount: state.defendant.pagecount,
    activePage: state.defendant.activePage,
    dataDefendant: state.defendant.defendant,
  };
}
