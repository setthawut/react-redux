...
const AppealFilterAdmin = (props) => {
  const { handleSubmit, loading, getAppeal, reset, dataAppeal, keyType } =
    props;
  const [stateType, setStateType] = useState("");
  const mapFilterAppealAdminToRequest = (values) => {
    let findType = dataAppeal.find((item) => item.type);
    if (!!findType) {
  ...
  const submitRenderFilter = async (values) => {
    ...
    await getAppeal(mapFilterAppealAdminToRequest(values));
  };
  return (
